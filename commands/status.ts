...
export function updateLastSelfCheck(): void {
...
async function checkCitizenSheet(): Promise<string> {
  try {
...
async function checkBlacklistSheet(): Promise<string> {
  try {
    const auth = new JWT({
...
async function checkMojang(): Promise<string> {
  try {
...
async function checkBedrock(): Promise<string> {
  try {
...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  const blacklistSheet =
    results[1].status === 'fulfilled' ? results[1].value : '⛔ ブラックリスト：連携失敗';
