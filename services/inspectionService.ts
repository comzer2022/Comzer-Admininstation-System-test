...
import type { Session, ParsedApplication, InspectionResult, CompanionEntry } from '../types/domain.js';
...
export async function runInspection(
  ...
  try {
    ...
    parsed = JSON.parse(rawContent) as ParsedApplication;

    if (parsed.companions && Array.isArray(parsed.companions)) {
      parsed.companions = parsed.companions.map((c: CompanionEntry) =>
  ...
  if (parsed.companions && Array.isArray(parsed.companions)) {
    for (const companion of parsed.companions) {
      ...
      try {
        ...
        const resp = await axios.get(url, { validateStatus: () => true });
        companionExists = version === 'java' ? resp.status === 200 : resp.data?.success === true;
      } catch (err) {
  ...
  if (periodHours > 24 * 31) {
    return {
      approved: false,
      content:
        '申請期間が長すぎるため却下します（申請期間が31日を超える場合、31日で申請後、申請が切れる前に再審査をお願いいたします。）',
      parsed,
    };
  }
