...
export class DebugCommand {
    ...
    constructor(private readonly debugMode: DebugModeState) { }
    async execute(interaction: ChatInputCommandInteraction): Promise<unknown> {
        const member = interaction.member;
        const hasPermission = member && 'cache' in member.roles
        ...
        this.debugMode.set(choice === 'on');
        const statusText = this.debugMode.isEnabled() ? 'ON' : 'OFF';
