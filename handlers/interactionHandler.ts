...
// 合流者確認ボタン応答
async function handleJoinerResponse(interaction: ButtonInteraction): Promise<unknown> {
...
async function handleVersionSelect(interaction: StringSelectMenuInteraction): Promise<unknown> {
...
async function handleButtonInteraction(interaction: ButtonInteraction): Promise<void> {
  ...
  if (type === 'cancel') {
    ...
    await endSession(session.id, 'キャンセル', interaction.client);
    return;
  }
...
// Modal 送信処理
async function handleModalSubmit(interaction: ModalSubmitInteraction): Promise<unknown> {
  ...
  const companions =
    companionsRaw && companionsRaw !== 'なし'
      ? companionsRaw
          .split(',')
          .map((x) => x.trim())
          .filter(Boolean)
  ...
  session.logs.push(
    `[${nowJST()}] 期間: ${period}, 同行者: ${companions.join(',') || 'なし'}, 合流者: ${joiner || 'なし'}`
  );
...
// Embed 生成ヘルパー
function createApprovalEmbed(parsed: ParsedApplication): EmbedBuilder {
  const today = new Date().toISOString().slice(0, 10);
  const safeReplace = (s: unknown): string | undefined =>
    typeof s === 'string' ? s.replace(/__TODAY__/g, today) : (s as string | undefined);

  const companionStr =
    Array.isArray(parsed.companions) && parsed.companions.length > 0
      ? parsed.companions
      ...
      : 'なし';
  const joinerStr =
    Array.isArray(parsed.joiners) && parsed.joiners.length > 0 ? parsed.joiners.join(', ') : 'なし';
  ...
  return new EmbedBuilder()
    ...
    .addFields(
      { name: '申請者', value: parsed.mcid ?? '不明', inline: true },
      { name: '国籍', value: parsed.nation ?? '不明', inline: true },
      { name: '申請日', value: nowJST(), inline: true },
      { name: '入国目的', value: safeReplace(parsed.purpose) ?? '不明', inline: true },
      {
        name: '入国期間',
...
function createRejectionEmbed(parsed: ParsedApplication, reasonMsg: string): EmbedBuilder {
  const companionStr =
    Array.isArray(parsed.companions) && parsed.companions.length > 0
      ? parsed.companions
          .map((c) => (typeof c === 'string' ? c : c.mcid))
...
async function publishApproval(
  ...
  const publishChannelId: string =
    (debugCommand.isDebugMode
      ? config.debugChannelId || LOG_CHANNEL_ID
      : config.publishChannelId || config.logChannelId || LOG_CHANNEL_ID) as string;
